...
export const getAllRecords = async (req, res) => {
  try {
    ...
    const countQuery = `SELECT COUNT(*) AS total FROM tbl_qualifications ${whereClause}`;
    ...
    return successResponse(res, "tbl_qualifications fetched successfully", {
      page,
      limit,
      total,
...
export const getRecords = async (req, res) => {
  try {
    ...
    // const [result] = await runQuery("SELECT * FROM tbl_qualifications WHERE id = ?", [id]);
...
export const createRecord = async (req, res) => {
  ...
  const [existing] = await runQuery("SELECT * FROM tbl_qualifications WHERE name = ?", [name]);
  ...
  try {
    const [result] = await runQuery(
      `INSERT INTO tbl_qualifications (name, description) VALUES (?, ?)`,
...
export const updateRecord = async (req, res) => {
  ...
  try {
    const [existing] = await runQuery("SELECT * FROM tbl_qualifications WHERE id = ?", [id]);
    if (!existing.length) {
      return errorResponse(res, "Category not found", 404);
    }
...
export const deleteRecord = async (req, res) => {
  ...
  try {
    const [result] = await runQuery("DELETE FROM tbl_qualifications WHERE id = ?", [id]);
    if (result.affectedRows === 0) {
      return errorResponse(res, "Category not found", 404);
    }
