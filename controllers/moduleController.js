...
export const getAllRecords = async (req, res) => {
  try {
    ...
    const countQuery = `SELECT COUNT(*) AS total FROM modules ${whereClause}`;
    ...
    return successResponse(res, "modules fetched successfully", {
      page,
      limit,
      total,
...
export const getRecordById = async (req, res) => {
  ...
  try {
    const [result] = await runQuery("SELECT * FROM modules WHERE id = ?", [id]);
    if (!result.length) {
...
export const createRecord = async (req, res) => {
  ...
  const [existing] = await runQuery("SELECT * FROM modules WHERE name = ?", [name]);
...
export const updateRecord = async (req, res) => {
  ...
  try {
    const [existing] = await runQuery("SELECT * FROM modules WHERE id = ?", [id]);
    if (!existing.length) {
      return errorResponse(res, "Category not found", 404);
    }
...
export const deleteRecord = async (req, res) => {
  ...
  try {
    const [result] = await runQuery("DELETE FROM modules WHERE id = ?", [id]);
    if (result.affectedRows === 0) {
      return errorResponse(res, "Category not found", 404);
    }
